...
def send_iso_tp_message(bus, arb_id, expected_response_ids, data, is_extend_id):
    ...
    # 根据CAN类型设置最大单帧长度
    max_single_frame_size = 62 if is_fd else 7  # CAN FD支持更多数据 (62 = 64-2 for PCI)
    ...
    if len(data) <= max_single_frame_size:
        ...
        # 发送多帧
        can_id = arb_id
        ff_dl = len(data)  # 总数据长度
        ff_data = data[:62] if is_fd else data[:6]  # 首帧数据（CAN FD最多62字节，CAN最多6字节）
        ...
        remaining_data = data[62 if is_fd else 6:]  # 剩余数据
        frame_id = 1  # 帧 ID 初始化

        while remaining_data:
            # 根据CAN类型确定每帧最大数据量
            max_cf_size = 63 if is_fd else 7  # CAN FD每帧可携带更多数据
            cf_data = remaining_data[:max_cf_size]  # 每帧的数据
            remaining_data = remaining_data[max_cf_size:]  # 更新剩余数据
...
def receive_iso_tp_message(bus, arb_id, expected_response_ids):
    ...
    while True:
        ...
        if first_frame.data[0] >> 4 == 0x0:
            ...
            data = first_frame.data[2:]  # 获取首帧数据
...
def read_memory(bus, start_address):
    ...
    while True:
        ...
        if response:
            ...
            # 检查是否是最后一帧
            if len(data) < len(response.data):  # 已经是最后一帧或者数据较短
                break
